...
function Card({title,icon,number,action,time,percentage}) {
  ...
  return (
    ...
    <span className='p-1 bg-[#C5C5C540] w-8 h-7 flex justify-center items-center  '>{icon}</span>
     ...
     <div 
  className={`rounded-lg text-xs py-1 px-3 mr-3 ${
    action.includes('increase') 
      ? 'bg-green-700 bg-opacity-20 text-green-700' 
      : action.includes('decrease') 
      ? 'bg-red-700 bg-opacity-10 text-red-700' 
      : 'bg-yellow-400 bg-opacity-20 text-yellow-700'
  }`}
>
  {action.includes('increase') && (
    <>
